# CLS Well-Architected Assessment
...
Four-pillar read-only assessment for **Log service — retention, indexing, shipping, audit coverage.**
...
| Check | API | Pass |
|-------|-----|------|
| Log shipping | `DescribeShippers` | Critical logs shipped to COS/CKafka for DR |
| Index health | `DescribeIndex` | Search index configured for audit topics |
| Collection coverage | Machine groups / configs | Production hosts shipping logs |

## 3. Security Pillar [assessment-readonly]

| Check | Pass |
|-------|------|
| CAM scoped | Read vs write separated for auditors |
| Encryption | TLS in transit; SSE at rest where available |
| Sensitive data | PII masking in log pipeline documented |

## 4. Cost Pillar [assessment-readonly]

| Check | Pass |
|-------|------|
| Retention | Per-topic retention aligned to compliance |
| Index cost | Full-text index only where needed |
| Ingest volume | Sampling on verbose debug logs |

## 5. Efficiency Pillar [assessment-readonly]

| Check | Pass |
|-------|------|
| Alarms | Log-based alarms on error patterns |
| Automation | Agent/config as code for new hosts |
...
| `product` | `cls` |
| Finding `id` pattern | `cls-{rel|sec|cost|eff}-NNN` |

### Pillar → checklist map

| `pillars` key | Checklist source |
|---------------|------------------|
| `reliability` | §2 Reliability — shipping DR, index health |
| `security` | §3 Security — CAM, encryption, access scope |
| `cost` | §4 Cost — retention, index volume, sampling |
| `efficiency` | §5 Efficiency — collection automation, alarms |
      ...
      "findings": [
        {
          ...
          "severity": "High",
          "confidence": "HIGH",
          "title": "Production logset without retention policy",
          "evidence": "Log topic retention unlimited or > 90d without archive",
          "recommendation": "Set retention; ship cold logs to COS via shipper",
          "effort": "medium"
        }
      ]
    },
  ...
  "recommendations": [
    {
      "priority": "High",
      "pillar": "reliability",
      "action": "Set retention; ship cold logs to COS via shipper",
      "effort": "medium"
    }
  ],
  "trace": {
    "commands": ["tccli cls DescribeLogsets --Region ap-guangzhou (SecretKey=<masked>)"],
