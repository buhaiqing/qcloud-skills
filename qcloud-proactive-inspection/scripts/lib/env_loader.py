...
Self-contained variant for qcloud-proactive-inspection scripts; mirrors the
qcloud-copilot env_loader contract without depending on the `copilot` package.
"""
...
def _find_dotenv(start: Path | None = None) -> Path | None:
    ...
    for base in (start or Path.cwd(), _SCRIPTS_DIR):
        cur = base.resolve()
...
def ensure_runtime_env() -> None:
    """Entry-point hook for cruise_sniff / cruise_analyze before tccli operations."""
    load_project_dotenv()
