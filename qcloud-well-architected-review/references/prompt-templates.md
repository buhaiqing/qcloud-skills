...
> Score [`references/rubric.md`](rubric.md) §3 + §4 (WA Review).
...
> **TE-6:** Pre-flight / Critic rule checks are **canonical** in [`references/rubric.md`](rubric.md) §4 (WA Review — 5 rules). Do not duplicate gate text here.
...
Workers: [`worker-output-schema.md`](worker-output-schema.md) + Product Worker Registry in SKILL.md.
