...
class IsolationForestDetector(BaseDetector):
    ...
    def _compute_threshold(self) -> float:
        ...
        import numpy as np
        try:
            scores = self._forest.score_samples(
                np.random.randn(1000, 1) * self._forest._tree_variance[0] ** 0.5
                + self._forest._centre[0]
            )
            return float(np.percentile(scores, self.contamination * 100))
        except Exception:
            return 0.5
