...
| L29 | **测试污染产品资产时，回归与 CI 都会假绿/假红两不耽误——必须用 conftest 做 session 级 additive 快照，而不能只盯单个 tearDown。** 实证：CR-1 修复后跑 5 个测试文件，5 个都把 `thresholds.json` 写向产品路径但**全部忘记 `tearDown`**：15 键 → 1 键（恰好就是 `c15e11a` 引入污染时的同一 bug shape）。首次发现时改 `tearDown` 失败——因为是**多文件共享产品状态**，部分测试必须看到"上一个测试已留下的状态"，加 `tearDown` 让它们变成间歇失败。修法：`scripts/conftest.py`（`pytest_sessionstart` 快照 + `pytest_sessionfinish` 检测已知污染 pattern —— 单键 dict、深度匹配 committed 的子集——并 additive 恢复；合法的产品状态改动只 WARN 不覆盖） | ① **「写在哪儿」和「清在哪儿」是同一条契约的两端**：测试写产品资产就要做 session-end 保护，不能让 `tearDown` 单点失败拖垮整个 suite；② **additive restore 比强制 restore 更安全**：合法的人工持久化由 git 兜底（污染检测与 committed 比对，对差异 WARN 而不抹除）；非法污染按 known-good 路径抹除；③ **把污染 pattern 写成枚举**：本会话用 `{1 key, single-key, subset of committed}` 三判据检出 5/5 案例；任何修单文件 tearDown 的方案都属于"把症状治在错的层级"；④ 凡写产品树中受版本管理的文件，先 grep 该文件路径的所有写点，没有 session-end 保护前不要合并 |
| L30 | **「自洽自检」不只是单元测试的领域；规则若声明「检查 X」，那么规则的 source 也必须可用同一套基础设施工具被发现、被执行、被计数——否则 X 就是零接线的字面承诺。** 实证：CR-4 W5 「zero-wiring rule」（H-101）：脚本自洽可运行但调用图上 0 caller——既非 surface 调用（`make`/workflow）也非传递 import。W1 仅抓"自称 CI gate"的脚本，**抓不到从未自称 gate 的零调用脚本**——语料 67% 就是这种类型（CR-4 commit `6410b0a`）。修法：check_gate_wiring.py 加 W5 规则，**对每个 scripts/*.py 计算 `reverse_callers`（包含传递 import 与工作流/壳层间接引用），等于 0 即违规** | ① 任何新规则的"火/静默证明"必须**同套工具链**：本会话的 fire-proof 测试用 `pytest` 而 CI shell 片段也用 `pytest`，同一份代码两种 caller，避免"测试中绿 / CI 中静默"漂移；② **W5 与 W1 互补而非替代**：W1 = "self-declared gate"; W5 = "any script with 0 callers"——两者并集才是完整接缝；③ **规则自身的测试不算"0 caller"**：checker 自身 + checker 测试文件要显式从阈值计算中排除（见 L27 W3b）；④ ratio 阈值（67%）应当落进 `thresholds.json:zero_wiring_max_ratio`：本次会话未完成棘轮化，下次同类任务顺带做掉 |
| L31 | **CI 中 step 失败默认 abort 整个 job——所以"全 job 通过"≠"所有检查都跑过"；任何同时承担 fail-closed 责任的检查必须独占一个 job，否则上游任一步非零即可让它静默跳过。** 实证：CR-3 拆分前，`KPI gates` 是 `validate` job 的 **step 16 / 17**；GitHub Actions `actions/checkout` 之后的常规 `run: ...` 默认 `shell: bash`、non-zero exit 即 fail-fast。结果：step 1–15 任一黄（ruff pin 不匹配、validate_*.py 抛 `FileNotFoundError` 等）KPI step 就**根本没跑**，workflow badge 仍绿色——"全绿"等同于"上游没红"，与"门禁通过"毫无关系；green ≠ passed，绿的是工作流，跑没跑是另一件事。修法：216 行 workflow 内把 KPI 步骤提到独立 job `kpi-gates`（独立 `runs-on`、`needs:` 链），fail-closed 责任落点与 step 计数同步 | ① **"通过"在工作流中至少有两种语义**——job exit code 与每个 step 的实际 exit code——必须以 job 边界对齐 fail-closed 责任，不能让一个非关键 step 的 fail-fast 吞掉另一个 step 的失败；② **任何"不通过就阻塞 PR"的检查都应该是独立 job**（`needs: [preflight]`、`runs-on: ubuntu-latest` 与其它 job 同级），不要嵌在 `validate` 大复合 job 里；③ **拆 job 顺手拆权限**：KPI 跑 `gcloud` / `tccli` 凭据时，独立 job 才能 mount 最小权限 secret，不污染其它 step；④ **"全绿"徽章不可单独作合规证据**：review 时除查 badge 颜色，还要看该 job 的 step 列表里**关键检查是否真的在 step 列表里**——徽章绿但 step 跳过与徽章红一样糟糕 |
