...
`qcloud-skills` is a collection of 34 Tencent Cloud ops skills delivered as AI-agent
runbooks, governed by a heavy but sound quality stack: GCL (Generator-Critic-Loop)
...
- **Destructive detection**: harness classifies each action against a destructive-action
  dictionary + schema; sets `safety.destructive` automatically.
- **Token↔plan binding**: a confirmation token is generated and bound 1:1 to
  `plan_hash` of the specific execution plan; execution is refused unless token matches
  (KPI #2, 100% coverage).
...
- **Intent confusion matrix**: Registry emits confusion matrix from `router_decision`
  over a labeled intent set (KPI #7).
