...
- **F1** `confusion_matrix` labels with the **owning skill dir** (`select_top1(query).top1_skill == skill`), not the per-item `intent` key absent from 24/31 `eval_queries.json` in the executable registry (29/36 across the full corpus — a different numerator, not the same fraction); that lookup pinned top1/misdelegation to a structural `0.0` while the gate printed a different (14.72%) quantity and passed. Honest baseline: 14.59% per-skill avg, 12.40% pooled (46/371), 22/31 at 0.0 — 9.87% (46/466) across all 36 skill dirs. `_top1_has_intent` deleted; `eval_queries.json` needs no new field.
...
- **F3** KPI#7 is now thresholded from `assets/shared/thresholds.json` — `router_min_top1_accuracy` 0.28 (ratchet at the measured baseline **over the 16 scoreable skills only**; second pass revised 0.10 → 0.28 when the denominator was fixed), an explicit `router_max_misdelegation` 0.16 ceiling (replacing the derived `1 - target` budget, which had 4× headroom and was effectively unfireable), `router_min_registry_skills` 31 (so deleting an unroutable skill cannot raise the average) and `router_target_top1_accuracy` 0.70. Fails below the ratchet, above the ceiling, below the registry floor, or when fewer than half the registry skills yield a measurement; a below-target pass prints the gap instead of a bare ✅.
- **F4** KPI#1/#2 validate `evidence-*.json` **and** the real `evidence-local.jsonl` stream (the `.json` glob alone enforced 1 record of hundreds; the record count is machine-local and gitignored, so it is deliberately not quoted here). `validate_evidence_schema.py` reads JSONL line-by-line, a malformed line is a fail, and it reports record counts, not file counts.
- **F5** blocking CI step "KPI gates" in `.github/workflows/validate-skills.yml` (previously no workflow invoked the gate), which since the 2026-09 kpi-honesty pass grades a **committed fixture** (`scripts/fixtures/evidence/`, selected by `GATE_EVIDENCE_GLOB`) instead of the evidence directory — the unit-test step in the same job mints schema-valid records into it, and the old `GATE_REQUIRE_EVIDENCE=0` escape short-circuits before any file is read. The step is blocking but **executes conditionally**: it is step 16 of 17 named steps, and Actions aborts the job at the first non-`continue-on-error` failure, so a red preceding step (e.g. Ruff at step 1) skips it and the run yields no KPI verdict — a green run means "nothing upstream was red", never by itself that the gate ran or passed. **F6** +9 unit tests (`harness_router_test`, new `check_kpi_gates_test`). **F7/F8** the drift this CR created or revealed is closed in `kpi-pattern.md`, the KPI#7 runbook and AGENTS.md CADL `L24`.
