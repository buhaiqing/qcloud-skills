...
- [x] **P5.4** 增加并发写入、重复事件、幂等 key 和部分失败恢复测试。已新建 `copilot/idempotency.py` 提供 `with_idempotency_key(producer, idempotency_key)` (线程安全；同 key 第二次返回缓存 + was_duplicate=True)；新建 `copilot/sink_recovery.py` 提供 `emit_with_recovery(sink, observations)` (best-effort emit + 每失败 record 索引 + 错误类型，绝不 raise)。4 测试覆盖：(a) 单线程同 key dedup；(b) 8-thread × 20 calls 中恰好 1 真实 + 19 dedup；(c) 25 并发 ObservableSink.emit_observation 后 jsonl 行数与 id 集合匹配；(d) 第 3 个 emit 抛 OSError 时其余 4 个成功落盘。
