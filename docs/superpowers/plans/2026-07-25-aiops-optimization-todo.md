...
| P0-5 SLO/业务影响驱动根因排序 | **✅ 完成**（2026-08-02） | `qcloud-copilot/copilot/root_cause_rank.py`(BusinessContext/CandidateRootCause/RankResult/RootCauseRanker/impact_score/rank/adjust_priority/default_weights) + `scripts/root_cause_rank_aggregate.py` + 21 tests（GCL 2 Critics：公式/拓扑/业务影响/窗口调整/验收；修复 partial-weight renormalize、NaN/inf 守卫、impact clamp、core 优先于 maintenance）。设计：`docs/superpowers/specs/slo-root-cause-ranking-design.md` |
| P1-1 ~ P1-5 | `[ ]` 未开始 | |
| P2-1 ~ P2-4 | `[ ]` 未开始 | |
