...
- [x] 修改 `qcloud-copilot/copilot/models.py`
  ...
  - **DoD**: `ruff check` 零 error；`test_models.py` 通过 ✅ (commit 0fbef97)
...
- [x] 修改 `qcloud-copilot/copilot/dispatcher.py`
  ...
  - 最大嵌套深度限制: 3 层
  - **DoD**: 集成测试验证条件分支逻辑 ✅ (commit 0fbef97)

### Step 2.1.3 — Plan Revision

- [x] 修改 `qcloud-copilot/copilot/dispatcher.py` 和 `planner.py`
  - `discovery` step 执行后触发 `plan_revision()`
  - `planner.replan()` 基于新发现重新规划剩余步骤
  - 安全约束: 不可撤销已完成的步骤
  - 最大 revision 次数: 3
  - **DoD**: discovery step 发现新错误码 → 剩余步骤重新规划 ✅ (commits 0fbef97 + c00cca4)

### Step 2.1.4 — 测试

- [x] 新增 `test_adaptive_workflow.py`
  - 条件分支 true 路径
  - 条件分支 false 路径
  - Plan revision 正确性
  - 最大 revision 次数限制
  - 安全约束: 已完成步骤不可撤销
  - **DoD**: 5+ 个测试通过 ✅ (14 tests, commit f1afcb7)
...
- [x] 新增 `qcloud-copilot/copilot/goal_inference.py`
  - `InferredGoal` + `SkillChain` dataclass
  - `GoalInference` 类: infer() 方法
  - LLM 调用 + SkillRegistry 匹配 + 风险评估
  - 安全约束: 破坏性方案标注 risk + 警告
  - **DoD**: `ruff check` 零 error ✅ (commit f52ce9d)

### Step 2.2.2 — Classifier 集成

- [x] 修改 `qcloud-copilot/copilot/classifier.py`
  - 模糊查询（confidence < 阈值）→ 触发 GoalInference
  - 明确指令（confidence > 阈值）→ 走现有分类逻辑
  - **DoD**: 模糊查询走目标推理，明确指令走现有逻辑 ✅ (commit f52ce9d)

### Step 2.2.3 — Engine 集成

- [x] 修改 `qcloud-copilot/copilot/engine.py`
  - `ask()` 增加目标推理路径
  - 多方案呈现 → 用户选择 → 生成 ExecutionPlan
  - **DoD**: "这台机器最近很慢" → 多方案 → 用户选择 → 执行 ✅ (commit f52ce9d)

### Step 2.2.4 — 测试

- [x] 新增 `test_goal_inference.py`
  - 模糊查询返回多方案
  - 破坏性方案标注风险
  - 低置信度返回 clarifying_questions
  - Skill 链验证
  - **DoD**: 4+ 个测试通过 ✅ (19 tests, commit f52ce9d)
...
- [x] 修改 `qcloud-copilot/copilot/engine.py`
  ...
  - **DoD**: FinOps HIGH → 自动 F1 ✅ (commit 9175769)

### Step 2.3.3 — 测试

- [x] 新增 `test_orchestration.py`
  - F1 触发条件匹配
  - P1 触发条件匹配
  - 多模式冲突 → specificity 选择
  - 无匹配 → None
  - **DoD**: 4+ 个测试通过 ✅ (6 tests, commit 9175769)
...
- [x] 新增 `qcloud-copilot/copilot/impact_analyzer.py`
  ...
  - **DoD**: `ruff check` 零 error ✅ (commit ea9fd53)
...
- [x] 修改 `scripts/harness_safety.py`
  ...
  - **DoD**: 风险分级确认逻辑正确 ✅ (commit ea9fd53)

### Step 2.4.3 — evidence_kernel.py 增强

- [x] 修改 `scripts/evidence_kernel.py`
  - human confirmation 提示增加影响面报告
  - 包含关联资源列表 + 建议预操作步骤
  - **DoD**: human confirmation 提示包含完整影响面 ✅ (merged via impact_analyzer integration)

### Step 2.4.4 — 测试

- [x] 新增 `test_impact_analyzer.py`
  - 无关联资源 → LOW
  - 2 个关联资源 → MEDIUM
  - 5+ 个关联资源 → HIGH/CRITICAL
  - 影响面报告完整性
  - **DoD**: 4+ 个测试通过 ✅ (12 tests, commit ea9fd53)
...
- [x] 条件分支: diagnose → fix-vpc (条件满足时) ✅
- [x] Plan revision: discovery step → 重新规划 ✅
- [x] "这台机器最近很慢" → 多方案 → 用户选择 → 执行 ✅
- [x] 破坏性方案标注 risk + 警告 ✅

### M2: 自主编排 + 预测性安全就绪（Month 2）

- [x] FinOps HIGH → F1 自动编排 ✅
- [x] 巡检 CRITICAL → P1 自动编排 ✅
- [x] 删除未挂载空磁盘 → 自动确认 ✅
- [x] 删除生产实例 → Human token + 影响面报告 ✅

### M3: Phase 2 整体验收（Month 3）

- [x] 一次模糊查询 → 目标推理 → 多方案选择 → 自适应执行 → 安全确认 完整闭环 ✅
- [x] 所有新增测试通过；回归测试零 failure ✅ (570 scripts + 51 phase2 tests)
- [x] ADR、Spec、Plan 文档状态更新为 Accepted / Complete ✅
