# CAM Proactive Inspection

> **delegate-from:** `qcloud-proactive-inspection` — read-only discovery/collection for **cam**.
...
|----------|-------------|
| Inventory | `tccli cam ListUsers / ListPolicies` (paginate `--Limit 100`) |
...
| Config state | `ListUsers / ListPolicies` response fields (status, backup, expiry) |

## Detection rules

| Rule ID | Check | Warning | Critical |
|---------|-------|---------|----------|
| cam-001 | Access key age | >90d | >180d |
| cam-002 | Wildcard policy | action:* present | Admin * on sub-account |
...
| `product` | `cam` |
| Finding `id` | `cam-NNN` (matches rule ID) |
