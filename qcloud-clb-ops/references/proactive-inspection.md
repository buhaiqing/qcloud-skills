# CLB Proactive Inspection

> **delegate-from:** `qcloud-proactive-inspection` — read-only discovery/collection for **clb**.
...
|----------|-------------|
| Inventory | `tccli clb DescribeLoadBalancers` (paginate `--Limit 100`) |
...
| Config state | `DescribeLoadBalancers` response fields (status, backup, expiry) |

## Detection rules

| Rule ID | Check | Warning | Critical |
|---------|-------|---------|----------|
| clb-001 | Unhealthy backend ratio | >20% unhealthy | >50% unhealthy |
| clb-002 | Listener without health check | HTTP/TCP missing check | Production listener no check |
...
| `product` | `clb` |
| Finding `id` | `clb-NNN` (matches rule ID) |
